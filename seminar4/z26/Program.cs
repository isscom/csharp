...
int N = int.Parse(Console.ReadLine()); //Пользователь вводит число и оно идет в переменную N
Console.Write(N); // Выводим в строку N

int count = 0; // Объявляем переменную (определяет кол-во цифр в числе)

while (N != 0) // Пока число не равно (!=) 0
{
    N = N / 10 ; // Делим число на 10 (убираем 1 цифру)
    count++; // Прибавляем к счетчику единицу (кол-во единиц в счетчике - кол-во цифр)
} 

Console.Write($"-> {count}"); // Выводим кол-во единиц (результат)

// МОЖНО РЕШИТЬ ЧЕРЕЗ ВВЕДЕНИЕ МЕТОДА (ЧЕРЕЗ ФУНКЦИЮ): (см. задачу z26func)
